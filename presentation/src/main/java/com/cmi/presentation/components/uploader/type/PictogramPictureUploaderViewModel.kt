...
class PictogramPictureUploaderViewModel(
    ...
    private val updatePictogramUseCase: UpdatePictogramUseCase
    ...
    private val isEditable = contentType is PictureUploaderContentType.PictogramEditable
    ...
    private fun insertOrUpdate(pictogramModel: PictogramModel) = viewModelScope.launch {
        val pictogram = pictogramModel.toPictogram()

        if (isEditable) {
            updatePictogramUseCase.invoke(pictogram)
                .catch {
                    showMessage(MessageType.GeneralError)
                }.collect {
            ...
            addPictogramUseCase.invoke(pictogram)
                .catch {
                    showMessage(MessageType.GeneralError)
                }.collect {
                    showMessage(getSuccessMessageType())
    ...
    private fun isValidForm(
        ...
        if (!isEditable && (categoryId.orNegative < 0 || categoryName.isNullOrEmpty())) {
